...
int main()
{
    try
    {
...

        {
            ...
            named_context.register_infix_operator(runtime_symbol, [](int left, int right) { return left + right; }, k_additive);
            named_context.register_infix_operator(view_symbol, [](int left, int right) { return left + right; }, k_additive);
        ...
        alias_calculator.context().register_infix_operator("sum", [](int left, int right) { return left + right; }, k_additive, fx::Associativity::Left);
        ...
        fx::SMath::set_context(fx::builtin::default_math_context());
        fx::SMath::set_value("global_counter", 9);
        require(fx::SMath::evaluate("{global_counter} + 1") == 10, "SMath should use an explicit builtin-backed global context");
        require(fx::evaluate("1 + 2").is<int>(), "free evaluate should work when the builtin header is included");
