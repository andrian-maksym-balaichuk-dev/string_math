...
int main()
{
    string_math::Calculator calculator(string_math::builtin::default_math_context());
    auto& context = calculator.context();

    context.add_infix_operator(
        "avg", [](double left, double right) { return (left + right) / 2.0; }, string_math::builtin::default_math::precedence::additive,
